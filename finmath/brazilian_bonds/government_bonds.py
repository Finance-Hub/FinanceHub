...
class NTNB(object):
    ...
    def __init__(self,
        ...
        """
        Class constructor.
        This is a Brazilian inflation-linked government bond that pays coupons every six months
        :param expiry: bond expiry date
        :param rate: bond yield
        :param price: bond price
        :param updated_nominal_value: bond nominal value updated by inflation
        :param principal: bond principal
        :param coupon_rate: bond coupon rate
        :param ref_date: reference date for price or rate calculation
        """

        msg = 'Two of the parameters rate, price and updated_nominal_value must be not None!'
        assert (rate is not None and price is not None)\
               or (rate is not None and updated_nominal_value is not None)\
               or (price is not None and updated_nominal_value is not None), msg

        self.expiry = pd.to_datetime(expiry).date()
        self.ref_date = pd.to_datetime(ref_date).date()

        if updated_nominal_value is None:
            self.price = float(price)
            self.rate = float(rate)
            self.updated_nominal_value = principal * self.updated_nominal_value_from_price(principal, coupon_rate)
        else:
            self.updated_nominal_value = float(updated_nominal_value)

        interest = ((1. + coupon_rate) ** (1. / 2.) - 1.) * self.updated_nominal_value
        cash_flows = pd.Series(index=self.payment_dates(),
                               data=interest).sort_index()
        cash_flows.iloc[-1] += self.updated_nominal_value
    ...
    def updated_nominal_value_from_price(self, principal, coupon_rate):
        interest = lambda x: ((1. + coupon_rate) ** (1. / 2.) - 1.) * principal * x
        last_pay = lambda x: principal * x + interest(x)
        theor_p = lambda x: LTN(dc.following(self.expiry), rate=self.rate, principal=last_pay(x), ref_date=self.ref_date).price +\
                            sum([LTN(d, rate=self.rate, principal=interest(x), ref_date=self.ref_date).price
                                 for d in self.payment_dates() if d != dc.following(self.expiry)])
        error = lambda x: (self.price - float(theor_p(x)))
