...
bbg_raw_data_df.to_excel(writer,'bbg_data')

day_count_dc = [0,7,31,62] #these are the number of days for spot, 1 week, 1 month, and 2 month
...
forwards = forwards.fillna(method='ffill')

value_dates.to_excel(writer,'value_dates')
...
number_of_trades = 20 #so, we get a strip of forwards
...
days_to_mat_new_position = pd.Series(index=value_dates.index[:-number_of_trades],
                                     data=[(a-b).days for a,b in zip(value_dates['spot'].iloc[number_of_trades:],
                                                                     value_dates['spot'].iloc[:-number_of_trades])])
...
for d,d_minus_1 in zip(tracker_index.index[1:],tracker_index.index[:-1]):
    ...
    fwd_strikes.iloc[0] = np.interp(float(days_to_mat_new_position[d_minus_1]),
                                    list(day_counts_df.loc[d_minus_1].astype(float).values),
                                    list(forwards.loc[d_minus_1].values))
    ...
    if currency in ['JPY']:
        pnl = -notionals * (fwd_mtm - fwd_strikes) / fwd_mtm #these are for the cases where the currency is quoted like USDXXX
    else:
        pnl = notionals*(fwd_mtm/fwd_strikes-1.) #these are for the cases where the currency is quoted like XXXUSD


    pnl_df = pd.concat([pnl_df,pnl.to_frame(d)],join='outer',axis=1)
...
pnl_df.transpose().to_excel(writer,'pnl')
...
bbg_tracker = pd.Series(index=tracker_index.index)
bbg_tracker.iloc[0] = tracker_index.iloc[0]
...
for d in spot_index.index[1:]:
    ...
    bbg_tracker[d] = bbg_tracker.loc[:d].iloc[-2]*(bbg_tracker_ret)


df = pd.concat([bbg_tracker.to_frame('bbg_tracker'),
